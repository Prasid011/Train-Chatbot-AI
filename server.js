...
const kbPath = path.join(__dirname, 'data', 'knowledge_base.json');// path to the knowlege base file used for staff chatbot

// This is for db setup using pool
const pool = new Pool({
  user: 'postgres',   //Parses the URL-encoded form data
  host: 'localhost',
  database: 'postgres',
  password: 'admin',
  port: 5432,
...
// these are the midlewares
app.use(express.urlencoded({ extended: true }));// parses the URL-encoded form data
app.use(express.json());// Parses the  JSON requests

// code for session managment handling logins
app.use(session({
...
// these are the static fils which are pulled from the static folder
app.use('/static', express.static(path.join(__dirname, 'static')));
app.use(express.static(path.join(__dirname, 'public')));

// Routes for file handling
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'templates', 'index.html'));// user chatbot UI
});

app.get('/login', (req, res) => {
  res.sendFile(path.join(__dirname, 'templates', 'login.html'));// Login page
});

app.get('/signup', (req, res) => {
  res.sendFile(path.join(__dirname, 'templates', 'signup.html'));// Signup page
});
...
app.get('/logout', (req, res) => {
  req.session.destroy(err => {
    ...
    res.redirect('/login');// once you logout this will redirect you to login
  });
});

// Signing up a new user which is saved in the users colum in the db
async function insertUser(username, password) {
  const hashedPassword = await bcrypt.hash(password, 10);//bcrypt is used for hasing the user pasword
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (userResult.rows.length > 0) {
      ...
      //This is used for invalid logins
...
app.post('/chat', async (req, res) => {
  ...
  try {
    ...
    const botReply = data.reply;
      //will insert the bot and user chat history under the chat_history table in db
    await pool.query(
...
//the server will start and list on the specified port
app.listen(3000, () => {
